...
/** 
 * @brief Exception class for shared memory
 */
...
public:
	 ...
	 * @return A pointer to a \c const \c char*. The underlying memory is in posession of the \c Exception object. Callers \a must not attempt to free the memory.
	 */
...
public:
	...
	/**
	 * @brief Initialize the names of the shared memory and the corresponding semaphore
